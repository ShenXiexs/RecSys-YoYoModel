...
class TrainConfig:
    """
    关键改动：
    1) 使用 HSTU：model_modul -> models.hstu_main.model_fn
    2) 显式配置 seq_length（固定长度），供 HSTU 使用
    3) 在 train_params 中新增 "hstu" 超参，并将 d_model 与动态 Embedding 维度对齐
    """

    # ======================= 基本信息 =======================
    model_version = "Base_hstu"                                # 必填：版本名（影响配置/输出路径）
    model_modul   = "models.hstu_main.model_fn"               # ★改为 HSTU 的 Estimator 入口
    ...
    train_params = {
        ...
        # ★HSTU 超参（与动态 Embedding 的 dim 必须一致）
        "hstu": {
            "d_model": 12,            # ★Embedding 维度/模型隐藏维；需与 TFRA 动态表 dim 一致
            "num_layers": 4,          # HSTU 层数
            "num_heads": 3,           # HSTU 头数
            "d_ff": 64,              # FFN 中间维度
            "attn_dropout": 0.1,      # 注意力层 dropout
            "ffn_dropout": 0.1,       # FFN dropout
            "causal": True,           # 生成式自回归（因果掩码）
            "pool_mode": "target",    # "target"=取最后一步；或 "mean"=mask 平均
            "use_other_features": True  # 是否拼接非序列的 features Embedding
        },

        # 资源与动态表策略（与 baseline 对齐）
        "ps_num": 0,                  # 参数服务器个数（无多机多卡则设 0）
        "restrict": False,            # 是否启用 TFRA 的 restrict policy
        "l2_reg": 1e-6,               # L2 正则（可调）
    }
    ...
    upload_oss_path = "hstu_model_test"  #定义模型导出OSS路径                   #必填
