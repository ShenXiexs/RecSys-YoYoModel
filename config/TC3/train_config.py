...
class TrainConfig:
    ...
    train_params = {}
    ### downodps
    data_schema = ['user_id', 'requestid', 'combination_un_id', 'adslot_id_type', 'is_click', 'is_conversion',
                   'features']
    ...
    # 定义分桶且特征选择表
    binning_table_name = "da_algorithm_search_ad_feature_rank_sample_ctr_binning_di"
    partitions = "idx_date='{day}',feature_version='TC3',durations='1'"
    downodps_datas = pd.date_range("20250815", "20250824").map(lambda x: x.strftime("%Y%m%d")).tolist()
    ### config path
    ...
    upload_oss_path = "TC3_model"  #dnnmodel->定义模型导出OSS路径
    oss_offline_root_path = "deep_model/offline"  # 离线特征推送OSS路径，判断特征是否推线上，再推模型到OSS供线上推理使用
    # 模型训练指标写入表,ocpc|cp模型
    metric_table = 'ads_yoyo_self_built_algorithm_model_eval_metric_table_dm'