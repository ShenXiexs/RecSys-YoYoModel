...
# @Author : huangmian
# @File : run_datawork_check_features_and_metric
...
#定义模型分支号
model_version = "O35"
metric_model_version = "O35_mutil_cvr_v10"
    ...
    while True:
        ...
        try:
            ...
            if elapsed_time >= max_wait_time:
                ...
                msg = f"{file_name}-{line_num} {exc_type.__name__}:{e}"
                logger().error(msg)
...
def check_eval_metric_isok():
    ...
    #         metric_value = float(model_eval_metric_dict[metric_nm])
    #         if min_threshold <= metric_value <= max_threshold:
    #             logger().info(f"---{metric_nm}={metric_value}, measure result is OK---")
    #         else:
    #             raise ValueError(f"指标不符合上线阈值, "
    #                              f"{metric_nm}={metric_value}不在阈值范围内: {metric_threshold}"
    #                             )
    #     else:
    #         raise ValueError(f"---{metric_nm} not in {list(model_eval_metric_dict.keys())}---")
    # 模型达标生成标记文件
    bucket.put_object(feature_file_off_p, "SUCCESS")
    logger().info("-----------------check_eval_metric_isok--------------------------")
