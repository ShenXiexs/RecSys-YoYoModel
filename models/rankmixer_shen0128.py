# models/rankmixer_shen0128.py
# RankMixer_Shen0128 Estimator: strict paper-style token mixing + per-token FFN (+ optional MoE).
...
# ===== Logging helpers =====
def _log_feature_list(title, items, per_line=20):
    items = list(items or [])
    logger.info("%s_count=%d", title, len(items))
    if not items:
        return
    for i in range(0, len(items), per_line):
        chunk = items[i:i + per_line]
        logger.info("%s[%d:%d]=%s", title, i, i + len(chunk) - 1, ", ".join(chunk))
...
def _safe_gather(emb_u, idx):
    # Guard against rare out-of-range indices from dynamic embedding lookup.
    emb_dim = tf.shape(emb_u)[1]
    emb_u = tf.concat([emb_u, tf.zeros([1, emb_dim], dtype=emb_u.dtype)], axis=0)
    size = tf.shape(emb_u)[0]
    idx = tf.cast(idx, tf.int64)
    size64 = tf.cast(size, tf.int64)
    size64 = tf.maximum(size64, 1)
    # Use modulo to guarantee indices are in range even if idx == size.
    idx = tf.math.floormod(idx, size64)
    return tf.gather(emb_u, tf.cast(idx, tf.int32), validate_indices=False)
...
def _get_seq_embedding(tokens_2d, embeddings_table, policy, name="seq_lookup"):
    ...
    ids = tf.strings.to_hash_bucket_strong(flat, 2 ** 63 - 1, [1, 2])
    update_tstp_op = policy.apply_update(ids)
    restrict_op = policy.apply_restriction(int(1e8))
    emb_flat, _ = de.embedding_lookup(embeddings_table, ids, return_trainable=True, name=name)
    seq_emb = tf.reshape(emb_flat, [batch_size, length, embeddings_table.dim])
...
def _get_dense_emb_from_features(features, embeddings_table, policy):
    ...
    update_tstp_op = policy.apply_update(ids)
    restrict_op = policy.apply_restriction(int(1e8))
    emb_flat, _ = de.embedding_lookup(embeddings_table, ids, return_trainable=True, name="features_lookup")
    dense_emb = tf.reshape(emb_flat, [batch_size, fea_size * embeddings_table.dim])
...
def model_fn(features, labels, mode, params):
    ...
    require_seq_in_tokenization = bool(rank_cfg.get("require_seq_in_tokenization", True))
    require_seq_coverage = bool(rank_cfg.get("require_seq_coverage", False))
    require_dense_coverage = bool(rank_cfg.get("require_dense_coverage", False))
    coverage_log_limit = int(rank_cfg.get("coverage_log_limit", 20))
    ...
    seq_token_count_raw = int(seq_token_count)

    # Log all feature names used by the model (dense + seq) at graph build time.
    _log_feature_list("RankMixer_Shen0128.dense_features", dense_feature_names, per_line=20)
    _log_feature_list("RankMixer_Shen0128.seq_tokens", seq_names, per_line=10)

    if require_seq_in_tokenization and seq_token_count_raw == 0:
        raise ValueError("Seq tokens required but none were prepared. Check seq_features_config and input data.")
    if require_seq_in_tokenization and seq_token_count_raw > 0 and not include_seq_in_tokenization:
        raise ValueError("Seq tokens exist but include_seq_in_tokenization=False while require_seq_in_tokenization=True.")

    if tokenization != "semantic":
        raise ValueError("RankMixer_Shen0128 expects semantic tokenization.")

    if token_mixing_type not in ("paper_strict", "paper", "param_free"):
        raise ValueError("RankMixer_Shen0128 only supports parameter-free token mixing.")
    ...
    if tokenizer_cls is SemanticTokenizerV3:
        tokenizer_kwargs.update(
            require_seq_coverage=require_seq_coverage,
            require_dense_coverage=require_dense_coverage,
            coverage_log_limit=coverage_log_limit,
        )
    ...
    coverage_info = getattr(tokenizer, "coverage_info", {}) if tokenizer is not None else {}
    seq_missing_count = int(coverage_info.get("seq_missing_count", 0))
    dense_missing_count = int(coverage_info.get("dense_missing_count", 0))
    if require_seq_coverage and seq_token_count_raw > 0 and seq_missing_count > 0:
        raise ValueError(
            "Seq coverage required but missing seq tokens remain. Missing count=%d sample=%s"
            % (seq_missing_count, ", ".join(coverage_info.get("seq_missing_sample", [])[:coverage_log_limit]))
        )
    if require_dense_coverage and dense_missing_count > 0:
        raise ValueError(
            "Dense coverage required but missing dense features remain. Missing count=%d sample=%s"
            % (dense_missing_count, ", ".join(coverage_info.get("dense_missing_sample", [])[:coverage_log_limit]))
        )
    ...
    if not per_token_ffn and not use_moe:
        raise ValueError("RankMixer_Shen0128 expects per-token FFN (or MoE).")
    ...
    loggings = OrderedDict({
        ...
        "rankmixer_seq_tokens": tf.constant(int(seq_token_count_raw), dtype=tf.int32),
    })
    if isinstance(features, dict):
        if "__pad_count" in features:
            loggings["data_pad_count"] = tf.cast(features["__pad_count"], tf.int64)
        if "__bad_field_count" in features:
            loggings["data_bad_field_count"] = tf.cast(features["__bad_field_count"], tf.int64)
        if "__bad_dense_count" in features:
            loggings["data_bad_dense_count"] = tf.cast(features["__bad_dense_count"], tf.int64)
    if coverage_info:
        loggings["rankmixer_seq_coverage"] = tf.constant(float(coverage_info.get("seq_coverage_ratio", 0.0)), dtype=tf.float32)
        loggings["rankmixer_seq_missing"] = tf.constant(int(coverage_info.get("seq_missing_count", 0)), dtype=tf.int32)
        loggings["rankmixer_dense_coverage"] = tf.constant(float(coverage_info.get("dense_coverage_ratio", 0.0)), dtype=tf.float32)
        loggings["rankmixer_dense_missing"] = tf.constant(int(coverage_info.get("dense_missing_count", 0)), dtype=tf.int32)
    if use_conditional_cvr:
    ...
    if l2_reg > 0:
        def _skip_l2(var_name):
            if "bias" in var_name:
                return True
            # Skip dynamic embedding lookup variables to avoid gather out-of-range in TFRA.
            for token in ("_lookup/", "lookup_table", "dynamic_embedding", "cuckoo", "embeddings_mht"):
                if token in var_name:
                    return True
            return False

        l2_vars = [v for v in trainable_variables if not _skip_l2(v.name)]
        if l2_vars:
            l2_loss = l2_reg * tf.add_n([tf.nn.l2_loss(tf.cast(v, tf.float32)) for v in l2_vars])
    ...
    optimizer_name = str(opt_cfg.get("optimizer", "adam")).lower()
    if optimizer_name in ("sgd", "gd", "gradientdescent"):
        opt = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=lr)
    elif optimizer_name in ("adagrad", "adag"):
        init_acc = float(opt_cfg.get("initial_accumulator_value", 1e-8))
        opt = tf.compat.v1.train.AdagradOptimizer(learning_rate=lr, initial_accumulator_value=init_acc)
