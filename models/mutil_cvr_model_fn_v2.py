...
def ctcvr_fusion(pctr, pcvr, type="multiply", hidden_units=None, name=''):
    logger.info(f"------{name}-ctcvr_fusion.type={type}-----------")
    if type == 'concat':
        fusion_input = tf.concat([tf.expand_dims(pctr, axis=1), tf.expand_dims(pcvr, axis=1)], axis=-1)
        pctcvr = tf.compat.v1.layers.dense(inputs=fusion_input, units=1, name=f'{name}_pctcvr_fusion_layer')
        pctcvr = tf.sigmoid(pctcvr)
    elif type == 'mlp':
        fusion_input = tf.concat([pctr, pcvr], axis=-1)
        mlp = MLP(hidden_units, output_dim=1, output_activation="sigmoid")
        pctcvr = mlp(fusion_input)
    else:
        pctcvr = pctr * pcvr
    return tf.reshape(pctcvr, [-1])

def build_layers(units, prefix, activation=None):
        act = "relu"
        layers = []
...
def model_fn(features: dict, labels, mode, params: dict):
    ...
    with tf.name_scope("awake_tower"):
        ...
        # mlp2 = MLP(**params.get("mlp_config", {}))
        ...
        # awake_ctcvr = ctcvr_fusion(ctr_logits, awake_logits, params.get("awake_fusion_type", "multiply"), name='awake')
